...
exports.getAllUsers = catchAsync(async (req, res, next) => {
    ...
    const suggestedUsers = users
        .filter((u) => !u.followers.includes(req.user._id) && u._id.toString() !== req.user._id.toString())
        .slice(0, 5)
        .reverse();
...
exports.deleteProfile = catchAsync(async (req, res, next) => {
    ...
    for (let i = 0; i < following.length; i++) {
        ...
        if (index > -1) follows.followers.splice(index, 1);
        await follows.save();
    }
...
exports.followUser = catchAsync(async (req, res, next) => {
    ...
    if (!loggedInUser) {
        return next(new ErrorHandler("Logged in user not found", 404));
    }

    // Safe defaults for old documents
    loggedInUser.following = Array.isArray(loggedInUser.following) ? loggedInUser.following : [];
    userToFollow.followers = Array.isArray(userToFollow.followers) ? userToFollow.followers : [];
    userToFollow.notifications = Array.isArray(userToFollow.notifications) ? userToFollow.notifications : [];
...
exports.getNotifications = catchAsync(async (req, res, next) => {
    const user = await User.findById(req.user._id)
        .populate("notifications.sender", "username avatar name")
