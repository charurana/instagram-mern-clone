...
const Inbox = () => {
    ...
    // session-based seen for latest own message
    const [lastOwnMessageSeen, setLastOwnMessageSeen] = useState(false);
    ...
    useEffect(() => {
        ...
        socket.current.on("messageSeen", ({ seenBy }) => {
            if (seenBy === userId) {
                setLastOwnMessageSeen(true);
            }
        });
    ...
    useEffect(() => {
        if (arrivalMessage && arrivalMessage.sender === userId) {
            dispatch({
    ...
    useEffect(() => {
        if (!loggedInUser?._id || !socket.current) return;
    ...
    // when this chat is open, tell sender their latest message has been seen
    useEffect(() => {
        if (!socket.current || !loggedInUser?._id || !userId || !messages?.length) return;

        const latestIncomingMessage = [...messages]
            .reverse()
            .find((m) => m.sender === userId);
    ...
    }, [messages, loggedInUser?._id, userId]);
                            ...
                            <div className="w-full flex-1 flex flex-col gap-1.5 overflow-y-auto overflow-x-hidden p-4">
                                {loading ? (
                                    ...
                                    messages.map((m, index) => {
                                        const isOwn = m.sender === loggedInUser._id;
                                        ...
                                        return (
                                            <React.Fragment key={m._id}>
                                                <Message
                                                    ownMsg={isOwn}
                                                    showSeen={isOwn && isLastMessage && lastOwnMessageSeen}
                                                    {...friend}
                                                    {...m}
                                                />
