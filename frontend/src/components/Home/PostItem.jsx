...
const PostItem = ({
   ...
   const finalImage =
       image && image.startsWith("http") ? image : BASE_POST_IMAGE_URL + image;
   ...
   const handleLikeModal = () => {
       if (setUsersDialog && setUsersList) {
           setUsersDialog(true);
   ...
   useEffect(() => {
       setSaved((allSavedBy || []).some((id) => id === user._id || id?._id === user._id));
   }, [allSavedBy, user._id]);
       ...
       <div className="flex flex-col border rounded bg-white relative">
           <div className="flex justify-between px-3 py-2.5 border-b items-center">
               <div className="flex space-x-3 items-center">
                   ...
                   <Link to={`/${postedBy.username}`} className="text-black text-sm font-semibold">
                       {postedBy.username}
                   </Link>
               </div>
           ...
           <div className="flex flex-col px-4 space-y-1 border-b pb-2 mt-2">
               ...
               <span onClick={handleLikeModal} className="font-semibold text-sm cursor-pointer">
                   {allLikes.length} likes
               </span>

               <div className="flex flex-auto items-center space-x-1">
                   <Link to={`/${postedBy.username}`} className="text-sm font-semibold hover:underline">
                       {postedBy.username}
                   </Link>
                   <span className="text-sm break-words">{caption}</span>
               </div>
                       ...
                       {allComments.map((c) => (
                           <div className="flex items-start mb-2 space-x-2" key={c._id}>
                               <img
                                   ...
                                   alt="avatar"
                               />
                               <Link to={`/${c.user.username}`} className="text-sm font-semibold hover:underline">
                                   {c.user.username}
                               </Link>
